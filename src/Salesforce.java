...
public class Salesforce {
	public static ArrayList<Level> getStudents() {
		...
		try {
			...
			QueryResult queryResults = connection.query("SELECT FirstName, LastName, Last_Class_Level__c, Home_Location_Code__c FROM Contact WHERE Last_Class_Level__c != null AND Last_Class_Level__c != '?' AND Last_Class_Level__c != 'O'");
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < s.length; i++) {
					...
					//System.out.println(c.getLast_Class_Level__c());
